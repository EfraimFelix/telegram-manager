...
const startGroup = /^\/start(?:@[A-Za-z0-9_]{1,32})?\s+([A-Za-z0-9_-]{1,64})$/;
export function extractConnectionAttempt(update: z.infer<typeof updateSchema>) {
  const msg = update.message;
  if (!msg || !["group", "supergroup"].includes(msg.chat.type) || !msg.from || msg.from.is_bot) return null;
  const match = (msg.text ?? "").trim().match(startGroup);
  if (!match) return null;
  return {
    code: match[1], telegramUserId: String(msg.from.id), externalId: String(msg.chat.id),
    chatType: msg.chat.type, messageId: String(msg.message_id),
  };
}
export function extractMessage(update: z.infer<typeof updateSchema>) {
  ...
  if (startGroup.test(text.trim())) return null;
