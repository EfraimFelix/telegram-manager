    ...
    <main id="main-content" className="main-content">
      ...
      <div className="page-content"><header className="page-heading"><div><p className="eyebrow">A CLEARER PICTURE</p><h1>{section === "Overview" ? "Your community, at a glance." : section}</h1><p>{section === "Overview" ? "Keep the conversation flowing. We’ll help with the noise." : section === "Rules & testing" ? "Say what belongs. Test what happens." : section === "Moderation log" ? "Every decision, with the context to understand it." : "Bring your Telegram community into your workspace."}</p></div><button className="secondary" disabled={disabled} onClick={refresh}>{loading ? "Loading…" : "↻ Refresh"}</button></header>
        ...
        {data && <>
          ...
          {section === "Connections" && <div className="connection-grid"><section className="panel"><div className="section-heading"><div><span className="eyebrow">STEP 01</span><h2>Connect your bot</h2></div><span className={`badge ${data.bot ? "matched" : ""}`}>{data.bot ? data.bot.status : "Not connected"}</span></div>
          ...
          </section><section className="panel"><span className="eyebrow">STEP 02</span><h2>Add your community</h2><p>Add the bot to your Telegram group as an administrator. Permissions are checked again when you enable the rules that need them.</p>
          ...
          </section><section className="panel moderation-control"><div><span className="eyebrow">STEP 03</span><h2>Enable moderation when you’re ready</h2><p>New communities start with moderation off. Review and test your rules first. Permissions are verified for the active rules.</p><small>{activeRules} enabled {activeRules === 1 ? "rule" : "rules"} · Review and no-match decisions never execute actions.</small></div><button className={community?.moderationEnabled ? "secondary" : "primary"} disabled={disabled || !community || (!community.moderationEnabled && !activeRules)} onClick={() => community && void mutate("toggleModeration", { communityId: community.id, enabled: !community.moderationEnabled }, community.moderationEnabled ? "Moderation turned off." : "Moderation turned on.")}>{pending === "toggleModeration" ? "Updating…" : community?.moderationEnabled ? "Turn moderation off" : "Turn moderation on"}</button></section>
